...
class ViewController: UIViewController , UIImagePickerControllerDelegate,UINavigationControllerDelegate , UITextFieldDelegate{
    ...
    //**********************buttons action*****************************************
    // Pick from camera OR phone
    func getImage (pickImageFrom: UIImagePickerController.SourceType)
    {
        let imagePicker = UIImagePickerController()
        imagePicker.delegate = self
        imagePicker.sourceType = pickImageFrom
        present(imagePicker, animated: true, completion: nil)
    }
    @IBAction func cameraButton(_ sender: Any) {
        getImage(pickImageFrom: .camera)
    }
    // the button responsibe for picking photo from gallery
    @IBAction func pickImage(_ sender: Any) {
        getImage(pickImageFrom: .photoLibrary)
    }
    ...
    func save() {
        // Create the meme
        _ = Meme(topText: topTextField.text!, bottomText: bottomTextField.text!, originalImage: uiImagePicker.image!, memedImage: MemedImageGenerated)
    }
    // generating the meme
    //firstly hidden the nav bar and tool bar then render view finally show nav and tool bar again
    func generateMemedImage() -> UIImage {
        self.topNavigationBar.accessibilityElementsHidden = true
        ...
        self.topNavigationBar.accessibilityElementsHidden = false
